"""HearthTracer - traces agent activity to JSONL files and streams to callbacks.
...
class HearthTracer:
    ...
    - token_update: Context window size update
    """
    ...
    def __init__(self, trace_dir: Path):
        ...
        Args:
            trace_dir: Directory for trace files (e.g., data/traces)
        """
    ...
    def _write_event(self, agent_name: str, event_type: str, data: dict[str, Any]) -> None:
        ...
        entry = {
            "timestamp": datetime.now(HEARTH_TZ).isoformat(),
            "agent": agent_name,
            "turn_id": turn_id,
            "event": event_type,
            **data
    ...
    def log_token_update(
        ...
        Args:
            ...
            token_count: Current context window size
    ...
    def load_recent_entries(
        ...
        """Load recent trace entries for an agent.

        Useful for populating TUI history on startup.

        Args:
            agent_name: Which agent's traces to load
            limit: Maximum entries to return

        Returns:
            List of trace entry dicts, most recent last
        """
        trace_file = self._get_trace_file(agent_name)
    ...
    def get_turn_history(
        ...
        Returns:
            List of events for that turn
        """
        entries = self.load_recent_entries(agent_name, limit=1000)
        return [e for e in entries if e.get("turn_id") == turn_id]
